...
function decideFileFromHunks(record, before, nextText) {
  const remaining = hunksFromTexts(before, nextText);
  const decided = record.hunks ?? {};
  if (!remaining.every((hunk) => decided[hunk.key] !== void 0)) return;
...
function sessionDir(root, cwd, sessionId) {
  const project = cwd === void 0 || cwd === "" ? join3(root, "_no-cwd") : join3(root, projectKey(cwd));
...
var LocalHistoryRuntime = class {
  ...
  async listTimeline(sessionId, cwd, path) {
    ...
    const project = join4(root, projectKey(cwd));
    let names;
    try {
  ...
  async syncSession(sessionId, cwd, hits) {
    ...
    if (settings.watchEnabled && cwd !== void 0 && cwd !== "") this.ensureWatcher(sessionId, cwd);
