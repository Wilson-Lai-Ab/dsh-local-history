...
var HistoryStore = class {
  ...
  gc(index, limits, now = Date.now()) {
    ...
    if (liveBytes > limits.maxBytes) {
      const saves = remaining().filter((record) => record.source === "save").sort((a, b) => b.bytes - a.bytes || byMtimeThenId(a, b));
    ...
    if (liveBytes > limits.maxBytes) {
      const resolved = remaining().filter((record) => record.source === "agent" && isDecided(record)).sort(byMtimeThenId);
...
// src/history/prior.ts
function samePath(a, b) {
  return a.replace(/\\/g, "/").toLowerCase() === b.replace(/\\/g, "/").toLowerCase();
}
async function newestDifferentPrior(store, records, path, exceptHash, beforeIndex = records.length) {
  ...
  for (const hit of hits) {
    try {
      const saved = await input.store.withLock(async () => {
        ...
        const current = await input.readCurrent(hit.path);
        if (typeof current.content === "string" && current.content.length > MAX_SNAPSHOT_BYTES) return void 0;
...
async function backfillAgentBefore(store) {
  return store.withLock(async () => {
    ...
    for (const [position, record] of index.records.entries()) {
      ...
      const missingBefore = record.kind === "add" && (record.beforeHash === null || record.beforeHash === emptyHash);
