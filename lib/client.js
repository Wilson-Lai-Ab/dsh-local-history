...
var en = {
  ...
  tooLargeFile: "File too large \u2014 content not retained",
  turn: "Turn {n}",
...
function roundAt(rounds, turn) {
  if (turn === void 0) return rounds.get("x");
...
function collectTreeRounds(sessions, sessionId) {
  const snapshot = sessions?.list?.getSnapshot?.() ?? {};
  const byId = snapshot.byId ?? {};
  const ids = treeSessionIds(byId, sessionId);
  const rounds = /* @__PURE__ */ new Map();
  for (const id of ids) {
    const window2 = sessions?.binding?.(id)?.eventSource?.getSnapshot?.();
    const entries = window2?.entries ?? [];
    const numbered = window2?.hasMore !== true;
...
function treeWindowHasMore(sessions, sessionId) {
  const byId = sessions?.list?.getSnapshot?.()?.byId ?? {};
  return treeSessionIds(byId, sessionId).some(
    (id) => sessions?.binding?.(id)?.eventSource?.getSnapshot?.()?.hasMore === true
  );
}
async function loadOlderPages(sessions, sessionId, maxPages) {
  let loaded = 0;
  while (loaded < maxPages) {
    const byId = sessions?.list?.getSnapshot?.()?.byId ?? {};
    const pending2 = treeSessionIds(byId, sessionId).map((id) => sessions?.binding?.(id)).filter((binding) => binding?.eventSource?.getSnapshot?.()?.hasMore === true).map((binding) => binding?.session?.loadOlder).filter((load) => typeof load === "function");
    if (pending2.length === 0) break;
    await Promise.all(pending2.map((load) => load().catch(() => void 0)));
    loaded += 1;
  }
  return loaded;
}
...
var OLDER_PAGE_BUDGET = 12;
function recordTurnUnmapped(record, rounds) {
  return record.turn !== void 0 && roundAt(rounds, record.turn) === void 0;
}
function byNewestChange(a, b) {
...
function groupByRound(records, rounds) {
  const map = /* @__PURE__ */ new Map();
  for (const record of records) {
    const owned = roundAt(rounds, record.turn);
    const key = owned !== void 0 && owned.sinceTurn !== void 0 ? `r${owned.sinceTurn}` : record.turn === void 0 ? "x" : `t${record.turn}`;
...
function ReviewApp(props) {
  ...
  const [windowRevision, setWindowRevision] = (0, import_react2.useState)(0);
  const olderPagesRef = (0, import_react2.useRef)(0);
  const cwd = scope.cwd;
  ...
  const rounds = (0, import_react2.useMemo)(
        ...
        return /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { className: "dsh_lh_group", children: [
          /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { className: "dsh_lh_groupHeader", children: [
            /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)("div", { className: "dsh_lh_groupMeta", children: [
              /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("span", { className: "dsh_lh_groupTurn", children: label === void 0 ? t("turnUnknown") : t("turn", { n: String(label) }) }),
            ...
            /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("div", { className: "dsh_lh_groupPrompt", children: owned === void 0 || owned.prompt === "" ? t("noPrompt") : promptPreview(owned.prompt) })
          ] }),
...
function DiffView(props) {
  ...
  const [mode, setMode] = (0, import_react6.useState)("edit");
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("div", { className: "dsh_lh_root dsh_lh_pane", "data-lh-diff": "", children: [
    (markdown || !decided) && /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("div", { className: "dsh_lh_reviewBar", children: [
      markdown && /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("div", { className: "dsh_lh_modeToggle", children: [
      ...
      !decided && /* @__PURE__ */ (0, import_jsx_runtime4.jsx)("span", { className: "dsh_lh_reviewHint", children: t("agentEdited") }),
      !decided && /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("div", { className: "dsh_lh_reviewActions", children: [
        /* @__PURE__ */ (0, import_jsx_runtime4.jsx)(
          "button",
          {
            type: "button",
    ...
    state.tooLarge === true ? /* @__PURE__ */ (0, import_jsx_runtime4.jsx)("div", { className: "dsh_lh_empty", children: t("tooLargeFile") }) : state.binary ? /* @__PURE__ */ (0, import_jsx_runtime4.jsx)("div", { className: "dsh_lh_empty", children: t("binaryFile") }) : showPreview ? /* @__PURE__ */ (0, import_jsx_runtime4.jsx)(PreviewBoundary, { fallback: t("loadFailed"), children: /* @__PURE__ */ (0, import_jsx_runtime4.jsx)("div", { className: "dsh_lh_md", children: (0, import_react6.createElement)(import_dsh_client_ui_primitives.MarkdownText, {
      text: state.after,
    ...
    }) }) }) : /* @__PURE__ */ (0, import_jsx_runtime4.jsxs)("div", { className: "dsh_lh_fileWrap", children: [
...
var PreviewBoundary = class extends import_react6.Component {
  ...
  render() {
    if (this.state.error !== null) {
      return /* @__PURE__ */ (0, import_jsx_runtime4.jsx)("div", { className: "dsh_lh_error", children: this.props.fallback });
    }
...
.dsh_lh_modeButton[data-active='true'] {
  background: var(--dsw-alias-bg-layer-1);
  color: var(--dsw-alias-label-primary);
}
.dsh_lh_md {
...
.dsh_lh_md h1 { font: var(--dsw-font-markdown-h1); margin: 24px 0 12px; }
.dsh_lh_md h2 { font: var(--dsw-font-markdown-h2); margin: 24px 0 12px; }
.dsh_lh_md h3 { font: var(--dsw-font-markdown-h3); margin: 24px 0 12px; }
.dsh_lh_md p { margin: 12px 0; }
.dsh_lh_md :where(ul, ol) { margin: 12px 0; padding-left: 18px; }
.dsh_lh_md hr { display: block; border: none; height: 0.5px; margin: 24px 0; background: var(--dsw-alias-border-l2); }
.dsh_lh_md blockquote { border-left: 2px solid var(--dsw-alias-label-caption); margin: 12px 0 0; padding-left: 14px; }
.dsh_lh_md pre { margin: 12px 0; font-family: var(--ds-font-family-code); overflow: auto; }
.dsh_lh_md :not(pre) > code {
...
.dsh_lh_md table { border-collapse: collapse; }
.dsh_lh_md th,
.dsh_lh_md td {
  padding: 8px 12px;
